...
// our instanced class.
ISXPreSetup("ISXEQAutoSize",ISXEQAutoSize);
...
unsigned int ISXEQAutoSizeXML=0;
...
bool ISXEQAutoSize::Initialize(ISInterface *p_ISInterface)
{	pISInterface=p_ISInterface;
	...
	WriteChatf("ISXEQAutoSize Loaded");
	return true;
}

// shutdown sequence
void ISXEQAutoSize::Shutdown()
{
	// save settings, if you changed them and want to save them now.  You should normally save
	// changes immediately.
	//pISInterface->ExportSet(ISXEQAutoSizeXML,XMLFileName);

	pISInterface->UnloadSet(ISXEQAutoSizeXML);
...
void ISXEQAutoSize::ConnectServices()
{
...
void ISXEQAutoSize::RegisterCommands()
{
	// add any commands
...
void ISXEQAutoSize::RegisterAliases()
{
	// add any aliases
}

void ISXEQAutoSize::RegisterDataTypes()
{
	// add any datatypes
	// pMyType = new MyType;
...
void ISXEQAutoSize::UnRegisterCommands()
{
	// remove commands
//	pISInterface->RemoveCommand("MyCommand");
}
void ISXEQAutoSize::UnRegisterAliases()
{
	// remove aliases
}
void ISXEQAutoSize::UnRegisterDataTypes()
{
	// remove data types
	//if (pMyType)
	//{
...
void ISXEQAutoSize::UnRegisterTopLevelObjects()
{
	// remove Top-Level Objects
//	pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQAutoSize::UnRegisterServices()
{
	// shutdown our own services
//	if (hISXEQAutoSizeService)
//		pISInterface->ShutdownService(this,hISXEQAutoSizeService);
}

void ISXEQAutoSize::LoadSettings()
{
	bool bChanged=false;

	// open the settings file, and store the ID number in ISXEQAutoSizeXML
	ISXEQAutoSizeXML=pISInterface->OpenSettings(XMLFileName);
...
Any set can be referred to by its ID.  The base set is stored in ISXEQAutoSizeXML.  Note that the settings
...
*/
	...
	if (unsigned int GeneralGUID=pISInterface->FindSet(ISXEQAutoSizeXML,"General"))
	{
