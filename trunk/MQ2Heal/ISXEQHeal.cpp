...
// The mandatory pre-setup function.  Our name is "ISXEQHeal", and the class is ISXEQHeal.
...
// our instanced class.
ISXPreSetup("ISXEQHeal",ISXEQHeal);
...
bool ISXEQHeal::Initialize(ISInterface *p_ISInterface)
{	pISInterface=p_ISInterface;
	...
	WriteChatf("ISXEQHeal Loaded");
	return true;
}

// shutdown sequence
void ISXEQHeal::Shutdown()
{
	// save settings, if you changed them and want to save them now.  You should normally save
	// changes immediately.
	//pISInterface->ExportSet(ISXEQHealXML,XMLFileName);

	pISInterface->UnloadSet(ISXEQHealXML);
...
void ISXEQHeal::ConnectServices()
{
...
void ISXEQHeal::RegisterCommands()
{
	// add any commands
...
void ISXEQHeal::RegisterAliases()
{
	// add any aliases
}

void ISXEQHeal::RegisterDataTypes()
{
	// add any datatypes
	// pMyType = new MyType;
...
void ISXEQHeal::UnRegisterCommands()
{
	// remove commands
//	pISInterface->RemoveCommand("MyCommand");
}
void ISXEQHeal::UnRegisterAliases()
{
	// remove aliases
}
void ISXEQHeal::UnRegisterDataTypes()
{
	// remove data types
	//if (pMyType)
	//{
...
void ISXEQHeal::UnRegisterTopLevelObjects()
{
	// remove Top-Level Objects
//	pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQHeal::UnRegisterServices()
{
	// shutdown our own services
//	if (hISXEQHealService)
//		pISInterface->ShutdownService(this,hISXEQHealService);
}

void ISXEQHeal::LoadSettings()
{
	bool bChanged=false;

	// open the settings file, and store the ID number in ISXEQHealXML
	ISXEQHealXML=pISInterface->OpenSettings(XMLFileName);
...
Any set can be referred to by its ID.  The base set is stored in ISXEQHealXML.  Note that the settings
...
*/
	...
	if (unsigned int GeneralGUID=pISInterface->FindSet(ISXEQHealXML,"General"))
	{
