...
#include "ISXEQRenoDPS.h"

// The mandatory pre-setup function.  Our name is "ISXEQRenoDPS", and the class is ISXEQRenoDPS.
...
// our instanced class.
ISXPreSetup("ISXEQRenoDPS",ISXEQRenoDPS);
...
unsigned int ISXEQRenoDPSXML=0;
...
bool ISXEQRenoDPS::Initialize(ISInterface *p_ISInterface)
...
WriteChatf("ISXEQRenoDPS Loaded");
return true;
}

// shutdown sequence
void ISXEQRenoDPS::Shutdown()
{
    // save settings, if you changed them and want to save them now.  You should normally save
    // changes immediately.
    //pISInterface->ExportSet(ISXEQRenoDPSXML,XMLFileName);

    pISInterface->UnloadSet(ISXEQRenoDPSXML);
...
void ISXEQRenoDPS::ConnectServices()
{
...
void ISXEQRenoDPS::RegisterCommands()
{
    // add any commands
...
void ISXEQRenoDPS::RegisterAliases()
{
    // add any aliases
}

void ISXEQRenoDPS::RegisterDataTypes()
{
    // add any datatypes
    // pMyType = new MyType;
...
void ISXEQRenoDPS::UnRegisterCommands()
{
    // remove commands
    //    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQRenoDPS::UnRegisterAliases()
{
    // remove aliases
}
void ISXEQRenoDPS::UnRegisterDataTypes()
{
    // remove data types
    //if (pMyType)
    //{
...
void ISXEQRenoDPS::UnRegisterTopLevelObjects()
{
    // remove Top-Level Objects
    //    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQRenoDPS::UnRegisterServices()
{
    // shutdown our own services
    //    if (hISXEQRenoDPSService)
    //        pISInterface->ShutdownService(this,hISXEQRenoDPSService);
}

void ISXEQRenoDPS::LoadSettings()
{
    bool bChanged=false;

    // open the settings file, and store the ID number in ISXEQRenoDPSXML
    ISXEQRenoDPSXML=pISInterface->OpenSettings(XMLFileName);
    ...
    Any set can be referred to by its ID.  The base set is stored in ISXEQRenoDPSXML.  Note that the settings
    ...
    */
    ...
    if (unsigned int GeneralGUID=pISInterface->FindSet(ISXEQRenoDPSXML,"General"))
    {
