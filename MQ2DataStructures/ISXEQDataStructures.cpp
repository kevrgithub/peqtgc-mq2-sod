...
#include "ISXEQDataStructures.h"

// The mandatory pre-setup function.  Our name is "ISXEQDataStructures", and the class is ISXEQDataStructures.
...
// our instanced class.
ISXPreSetup("ISXEQDataStructures",ISXEQDataStructures);
...
unsigned int ISXEQDataStructuresXML=0;
...
bool ISXEQDataStructures::Initialize(ISInterface *p_ISInterface)
{	pISInterface=p_ISInterface;
	...
	WriteChatf("ISXEQDataStructures Loaded");
	return true;
}

// shutdown sequence
void ISXEQDataStructures::Shutdown()
{
	// save settings, if you changed them and want to save them now.  You should normally save
	// changes immediately.
	//pISInterface->ExportSet(ISXEQDataStructuresXML,XMLFileName);

	pISInterface->UnloadSet(ISXEQDataStructuresXML);
...
void ISXEQDataStructures::ConnectServices()
{
...
void ISXEQDataStructures::RegisterCommands()
{
	// add any commands
...
void ISXEQDataStructures::RegisterAliases()
{
	// add any aliases
}

void ISXEQDataStructures::RegisterDataTypes()
{
	// add any datatypes
	// pMyType = new MyType;
...
void ISXEQDataStructures::RegisterTopLevelObjects()
{
	// add any Top-Level Objects
//	pISInterface->AddTopLevelObject("MapSpawn",tloMapSpawn);
}

void ISXEQDataStructures::RegisterServices()
{
	// register any services.  Here we demonstrate a service that does not use a
	// callback
	// set up a 1-way service (broadcast only)
//	hISXEQDataStructuresService=pISInterface->RegisterService(this,"ISXEQDataStructures Service",0);
	// broadcast a message, which is worthless at this point because nobody will receive it
	// (nobody has had a chance to connect)
//	pISInterface->ServiceBroadcast(this,hISXEQDataStructuresService,ISXSERVICE_MSG+1,0);

}

void ISXEQDataStructures::DisconnectServices()
{
...
void ISXEQDataStructures::UnRegisterCommands()
{
	// remove commands
//	pISInterface->RemoveCommand("MyCommand");
}
void ISXEQDataStructures::UnRegisterAliases()
{
	// remove aliases
}
void ISXEQDataStructures::UnRegisterDataTypes()
{
	// remove data types
	//if (pMyType)
	//{
...
void ISXEQDataStructures::UnRegisterTopLevelObjects()
{
	// remove Top-Level Objects
//	pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQDataStructures::UnRegisterServices()
{
	// shutdown our own services
//	if (hISXEQDataStructuresService)
//		pISInterface->ShutdownService(this,hISXEQDataStructuresService);
}

void ISXEQDataStructures::LoadSettings()
{
	bool bChanged=false;

	// open the settings file, and store the ID number in ISXEQDataStructuresXML
	ISXEQDataStructuresXML=pISInterface->OpenSettings(XMLFileName);
...
Any set can be referred to by its ID.  The base set is stored in ISXEQDataStructuresXML.  Note that the settings
...
*/
	...
	if (unsigned int GeneralGUID=pISInterface->FindSet(ISXEQDataStructuresXML,"General"))
	{
