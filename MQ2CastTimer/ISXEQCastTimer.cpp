...
#include "ISXEQCastTimer.h"

// The mandatory pre-setup function.  Our name is "ISXEQCastTimer", and the class is ISXEQCastTimer.
...
// our instanced class.
ISXPreSetup("ISXEQCastTimer",ISXEQCastTimer);
...
unsigned int ISXEQCastTimerXML=0;
...
bool ISXEQCastTimer::Initialize(ISInterface *p_ISInterface)
{	pISInterface=p_ISInterface;
	...
	WriteChatf("ISXEQCastTimer Loaded");
	return true;
}

// shutdown sequence
void ISXEQCastTimer::Shutdown()
{
	// save settings, if you changed them and want to save them now.  You should normally save
	// changes immediately.
	//pISInterface->ExportSet(ISXEQCastTimerXML,XMLFileName);

	pISInterface->UnloadSet(ISXEQCastTimerXML);
...
void ISXEQCastTimer::ConnectServices()
{
...
void ISXEQCastTimer::RegisterCommands()
{
	// add any commands
...
void ISXEQCastTimer::RegisterAliases()
{
	// add any aliases
}

void ISXEQCastTimer::RegisterDataTypes()
{
	// add any datatypes
	// pMyType = new MyType;
...
void ISXEQCastTimer::UnRegisterCommands()
{
	// remove commands
//	pISInterface->RemoveCommand("MyCommand");
}
void ISXEQCastTimer::UnRegisterAliases()
{
	// remove aliases
}
void ISXEQCastTimer::UnRegisterDataTypes()
{
	// remove data types
	//if (pMyType)
	//{
...
void ISXEQCastTimer::UnRegisterTopLevelObjects()
{
	// remove Top-Level Objects
//	pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQCastTimer::UnRegisterServices()
{
	// shutdown our own services
//	if (hISXEQCastTimerService)
//		pISInterface->ShutdownService(this,hISXEQCastTimerService);
}

void ISXEQCastTimer::LoadSettings()
{
	bool bChanged=false;

	// open the settings file, and store the ID number in ISXEQCastTimerXML
	ISXEQCastTimerXML=pISInterface->OpenSettings(XMLFileName);
...
Any set can be referred to by its ID.  The base set is stored in ISXEQCastTimerXML.  Note that the settings
...
*/
	...
	if (unsigned int GeneralGUID=pISInterface->FindSet(ISXEQCastTimerXML,"General"))
	{
